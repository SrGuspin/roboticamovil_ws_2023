...
class TurtlebotController(object):
    ...
    def obtacle_detected(self):
        ...
        if self.depth_image_np is not None:
            ...
            obstacle1 = bool(np.mean(columna1)*0.1 < 90 )
            obstacle2 = bool(np.mean(columna2)*0.1 < 50)
            obstacle3 = bool(np.mean(columna3)*0.1 < 90)
            columnas = Vector3(columna1, columna2, columna3)
            self.centro_tunel = (np.mean(columna3)*0.1 -
                                 np.mean(columna1)*0.1)/2  # - 1.25
    ...
    def detectar_imagen(self):
        ...
        #     '/home/grupo9/Desktop/grupo5/src/lab2_pkg/scripts/arrow_left.png'))
        if self.rgb_image_np is None:
            return 0
